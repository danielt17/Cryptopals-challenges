...
import numpy as np
from sets.set_01_basics.challenge_03_single_byte_xor_cipher.single_byte_xor_cipher import getAllcharctersEncodingInascii,SingleByteXorCipher,get_english_score
from sets.set_01_basics.challenge_05_repeating_key_xor.repeating_key_xor import repeatingKeyXOR
...
def HammingDistanceBytes(bytes1,bytes2):
    """Compute hamming distance bytes."""
...
def HammingDistanceStrings(str1,str2):
    """Compute hamming distance strings."""
    HammingLen = 0
...
def LoadCypherText():
    """Load cypher text."""
    with Path(__file__).with_name('16.txt').open('r', encoding='utf-8') as f:
...
def BruteForceSingleByteXorCipher(bitSeq1):
    ...
    """Brute force single byte xor cipher."""
    CHARACTER_FREQ = {
